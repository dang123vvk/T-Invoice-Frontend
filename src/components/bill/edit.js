...
class EditBill extends Component {
    ...
    componentDidMount() {
        ...
        //template
        getTemplateBill(this.props.match.params.id,this.props.token).then(data => {
            this.setState({
            ...
            if( data.status==true)
                {
                    if(( data.length > 0) && ( data.po_number.status_po_id == 2))
                    {
                        this.setState({ 
                            ...
                            templates_id: data.bill.templates_id,
                            ...
                            status_bill_id:  data.bill.status_bill_id,
                            customer_address:  data.bill.customer_address,
                            bill_monthly_cost:  data.bill.bill_monthly_cost,
                            bill_date:  data.bill.bill_date,
                            disabled: false,
                            bill_no:  data.bill.po_number_id,
    ...
    handleSubmitForm(event) {
        ...
        postBillUpdate(this.props.match.params.id,bill, this.props.token).then(data => {
