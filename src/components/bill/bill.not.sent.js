...
const API_URL_BILL_NOT_SENT = API + 'bills/list/user/length/notsent/';
class ListBillNotSent extends React.Component {
    _exporter;
    export = () => {
        this._exporter.save();
    }
    ...
    componentDidMount() {
        axios.get(API_URL_BILL_NOT_SENT + localStorage.getItem('user_id'), { headers: { Authorization: localStorage.getItem('token') } })
            .then(response => {
...
export default connect(mapStateToProps)(ListBillNotSent);