...
const API_URL = API + 'bills/senior/director/';
class ListBillSenior extends React.Component {
    ...
    componentDidMount() {
        axios.get(API_URL + this.props.match.params.id + '/' + this.props.match.params.user_id, { headers: { Authorization: localStorage.getItem('token') } })
            .then(response => {
                this.setState({
