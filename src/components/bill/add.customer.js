...
class AddBillCustomer extends Component {
    ...
    componentDidMount() {
        ...
        getCustomerForBill(this.props.match.params.id,this.props.token).then(data => {
            if(data.status_po_nos_add === true)
                {
                    this.setState({ 
                        customer_id: data.customers.customer_id,
                        ...
                        bill_reference: (this.state.month_1+data.customers.customer_swift_code).toUpperCase(),
                        swifcode: data.customers.customer_swift_code,
                        display_po: false,
                     });
                }
                else {
    ...
    customerdefault(e) {
        e.preventDefault();
        getTemplateCustomer(this.props.match.params.id,this.props.token).then(data => {
            this.setState({
                templates_name_company: data.temp.templates_name_company,
                templates_address: data.temp.templates_address,
                templates_phone: data.temp.templates_phone,
