...
const API_URL_BILL_LENGTH = API + 'bills/list/user/length/';
const API_URL_BILL_SUM = API + 'bills/list/user/sum/';
const API_URL_CUSTOMER_LENGTH = API + 'customers/length/';
const API_URL_BILL_NOT_SENT_LENGTH = API + 'bills/list/user/length/notsent/';
const API_URL_BILL = API + 'bills/list/user/limit/';
const API_URL_CUSTOMER = API + 'customers/limit/';
...
class DirectorDashboard extends Component {
    ...
    componentDidMount() {
        ...
        Axios.get(API_URL_BILL_LENGTH + localStorage.getItem('user_id'))
            .then(response => {
                this.setState({ billNumber: response.data.length });
            })
            .catch(err => console.log(err));
        Axios.get(API_URL_BILL_SUM + localStorage.getItem('user_id'))
            .then(response => {
                this.setState({ total: response.data.total });
            })
            .catch(err => console.log(err));
        Axios.get(API_URL_CUSTOMER_LENGTH + localStorage.getItem('user_id'))
            .then(response => {
                this.setState({ customerNumber: response.data.length });
            })
            .catch(err => console.log(err));
        Axios.get(API_URL_BILL_NOT_SENT_LENGTH + localStorage.getItem('user_id'))
            .then(response => {
                this.setState({ billNotSent: response.data.length });
            })
            .catch(err => console.log(err));
        if (this.props.token == '') {
            Axios.get(API_URL_BILL + localStorage.getItem('user_id'), { headers: { Authorization: localStorage.getItem('token') } })
                .then(response => {
                    this.setState({ dataBill: response.data.bill });
                })
                .catch(err => console.log(err));
            ...
            Axios.get(API_URL_BILL + localStorage.getItem('user_id'), { headers: { Authorization: this.props.token } })
                .then(response => {
                    this.setState({ dataBill: response.data.bill });
                })
                .catch(err => console.log(err));

            Axios.get(API_URL_CUSTOMER + localStorage.getItem('user_id'), { headers: { Authorization: this.props.token } })
                .then(response => {
                    this.setState({ dataCustomer: response.data.customers });
                })
                .catch(err => console.log(err));
    ...
    render() {
        ...
        if (redirect) {
          return <Redirect to={'/search/customer/'+ this.state.customer_name+ '/status/' + this.state.status_bill_id + '/date_from/'+this.state.date_from+ '/date_to/'+ this.state.date_to}/>;
        }
        return (
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs >
                                <Paper style={{ height: '100px', textAlign: 'center' }}>
                                    ...
                                    <Typography align="center">
                                        <Link to="/customer-list"><SupervisorAccountIcon style={{ fontSize: '50px', backgroundColor: '#2196f3', color: 'white' }} /></Link>
                                    </Typography>
                            ...
                            <Grid item xs >
                                <Paper style={{ height: '100px', textAlign: 'center' }}>
                                    <br />
                                    <Typography align="center">
                                        <Link to="/accountbank"><AccountBalanceIcon style={{ fontSize: '50px', backgroundColor: '#2196f3', color: 'white' }} /></Link>
                                    </Typography>
                                    <Typography variant="caption" className={this.classes.title} align="center">
                                        Account Bank
                        </Typography>
                            ...
                            <Grid item xs >
                                <Paper style={{ height: '100px', textAlign: 'center' }}>
                                    <br />
                                    <Typography align="center">
                                        <Link to="/bill-list"><ReceiptIcon style={{ fontSize: '50px', backgroundColor: '#2196f3', color: 'white' }} /></Link>
                                    </Typography>
                                    <Typography variant="caption" className={this.classes.title} align="center">
                                        Bills
                             </Typography>
                            ...
                            <Grid item xs hidden={!(this.props.isSenior)} >
                                <Paper style={{ height: '100px', textAlign: 'center' }}>
                                    <br />
                                    <Typography align="center">
                                        <Link to="/senior"><GroupWorkIcon style={{ fontSize: '50px', backgroundColor: '#2196f3', color: 'white' }} /></Link>
                                    </Typography>
                                    <Typography variant="caption" className={this.classes.title} align="center">
                                        Manage Group
                            </Typography>
